    ...
    <div class="w-full mx-auto py-5">
        <div class="bg-white p-8 rounded-lg shadow">
            ...
            <div class="flex items-center gap-5 mb-10">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Tambah Jadwal Wawancara</h1>
                    <p class="text-gray-600 mt-1">Tambahkan link dan jadwal wawancara</p>
                </div>
            </div>
            @if ($errors->any())
            ...
            <form id="jadwalCreateForm" action="{{ route('admin.jadwal-wawancara.store') }}" method="POST" class="border border-gray-200 rounded-2xl p-8">
                @csrf

                <div x-data="kampusJurusan()"
                    x-init='initData(
                        @json($cabangs),
                        @json((string)old("cabang_id")),
                        @json((string)old("jurusan_id"))
                    )'>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="gelombang_id" value="{{ old('gelombang_id') }}" class="w-full rounded-xl border-gray-300 px-4 py-3">
                        ...
                        <div>
                            ...
                            <select x-model="kampus" @change="updateJurusan()" name="cabang_id" class="w-full rounded-xl border-gray-300 px-4 py-3">
                                <option value="">Pilih Kampus</option>
                                <template x-for="c in cabangs" :key="c.id">
                                    <option :value="String(c.id)"
                                            x-text="c.nama_cabang">
                                    </option>
                                </template>
                            </select>
                        </div>
                        ...
                        {{-- Jurusan --}}
                        <div>
                            <label class="block text-sm mb-2"> Jurusan (Untuk Instruktur)</label>
                            <select name="jurusan_id" x-model="jurusan" :disabled="!kampus" class="w-full rounded-xl border-gray-300 px-4 py-3">
                                <option value="">Pilih Jurusan</option>
                                <template x-for="j in jurusanList" :key="j.id">
                                    <option :value="String(j.id)"
                                            x-text="j.nama_jurusan">
                                    </option>
                                </template>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="url" name="link_wawancara" value="{{ old('link_wawancara') }}"
                ...
                <div class="flex justify-end gap-3 mt-12">
                    ...
                    <button type="button"
                            onclick="confirmSubmit('jadwalCreateForm', 'Yakin ingin menyimpan jadwal wawancara?')"
