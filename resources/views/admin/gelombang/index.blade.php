        ...
        <div class="bg-white p-8 rounded-lg shadow">
            ...
            <div class="flex justify-between items-start mb-8">
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Control Setting Gelombang</h1>
                        <p class="text-gray-600 mt-1">Kelola jadwal pendaftaran, pretest, wawancara, dan pengumuman</p>
                    </div>
                </div>
                @if(auth()->user()->canCreateMenu('Gelombang'))
            ...
            <form action="{{ route('admin.gelombang.index') }}" method="GET"
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 text-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left font-semibold">Pendaftaran</th>
                            <th class="px-6 py-4 text-left font-semibold">Pretest</th>
                            <th class="px-6 py-4 text-left font-semibold">Durasi</th>
                            <th class="px-6 py-4 text-left font-semibold">Wawancara</th>
                            <th class="px-6 py-4 text-left font-semibold">Pengumuman</th>
                    ...
                    <tbody>
                        @forelse ($gelombangs as $gelombang)
                            <tr class="border-t hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-5">
                                    {{ $gelombang->tahun_periode }}
                                </td>

                                <td class="px-6 py-5">
                                    {{ $gelombang->pendaftaran_mulai?->format('d/m/Y H:i') ?? '-' }}
                                    <br>
                                    <span class="text-gray-400">s/d</span>
                                    {{ $gelombang->pendaftaran_selesai?->format('d/m/Y H:i') ?? '-' }}
                                </td>

                                <td class="px-6 py-5">
                                    {{ $gelombang->pretest_mulai?->format('d/m/Y H:i') ?? '-' }}
                                    <br>
                                    <span class="text-gray-400">s/d</span>
                                    {{ $gelombang->pretest_selesai?->format('d/m/Y H:i') ?? '-' }}
                                </td>

                                <td class="px-6 py-5">
                                    {{ $gelombang->durasi_pretest ?? '-' }} menit
                                </td>

                                <td class="px-6 py-5">
                                    {{ $gelombang->wawancara_mulai?->format('d/m/Y H:i') ?? '-' }}
                                    <br>
                                    <span class="text-gray-400">s/d</span>
                                    {{ $gelombang->wawancara_selesai?->format('d/m/Y H:i') ?? '-' }}
                                </td>

                                <td class="px-6 py-5">
                                    {{ $gelombang->pengumuman_mulai?->format('d/m/Y H:i') ?? '-' }}
                                </td>
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex items-center justify-center gap-4">
                                        @if(auth()->user()->canUpdateMenu('Gelombang'))
                                            <a href="{{ route('admin.gelombang.edit', $gelombang->id) }}"
                                            class="text-indigo-600 hover:text-indigo-800 transition">
                                                <i data-lucide="square-pen" class="w-5 h-5"></i>
                                            </a>
                                        @endif

                                        @if(auth()->user()->canDeleteMenu('Gelombang'))
                                            <form id="delete-gelombang-{{ $gelombang->id }}"
                                                action="{{ route('admin.gelombang.destroy', $gelombang->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')

                                                <button type="button"
                                                        onclick="confirmDelete('delete-gelombang-{{ $gelombang->id }}')"
